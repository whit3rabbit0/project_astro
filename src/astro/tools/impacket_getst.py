"""ImpacketGetSTTool — Kerberos delegation abuse — S4U2Self/S4U2Proxy."""
import re
...
class ImpacketGetSTTool(BaseTool):
    name = "impacket-delegation"
    description = "Kerberos delegation abuse — S4U2Self/S4U2Proxy"
    ...
    def validate(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        spn = params.get("spn", "")
        if not spn:
            raise ValueError("SPN parameter is required")
        if SHELL_METACHARACTERS.search(spn):
            raise ValueError("Invalid spn parameter")

        impersonate = params.get("impersonate", "")
        if not impersonate:
            raise ValueError("Impersonate parameter is required")
        if not all(c.isalnum() or c in ".-_" for c in impersonate):
            raise ValueError("Invalid impersonate parameter")
